\begin{tad}{\tadNombre{SimCity}}
    ...
    \vspace{2mm}
    \tadExporta{SimCity, observadores, generadores, turnos, nivelComercio, distManhattan, $\bullet\ \cup_{dicc}\ \bullet$}
    ...
    \tadOperacion{agComercios}{SimCity,dicc(Pos, Nivel),dicc(Pos, Construcción)}{dicc(Pos $\times$ Nivel)}{}
    %Calcula el nivel de un comercio al ser agregado al SimCity
    \tadOperacion{nivelComercio}{Pos,dicc(Pos, Nivel)}{Nat}{}
    ...
    \tadAlinearFunciones{construcciones}{dicc(Pos, Construcción),dicc(Pos, Construcción)}
    ...
    \tadAxioma{comercios(unir($s,\ s'$))}{
        ...
        \tab\tab\tab sacarRepes(construcciones($s$), construcciones($s'$)))
    }
    ...
    \paratodo{dicc(Pos $\times$ Nivel)}{cn}, \\
    ...
    %            \tab\tab\tab\tab nivelComercio(dameUno(claves($cs$)), casas($s$)), $cn$), \\
    ...
    \tadAxioma{nivelComercio($p,\ cn$)}{
        ...
        %        \tab\tab\tab nivelComercio($p$, borrar(dameUno(claves($cn$)), $cn$))) \\
        %    \tab $\LELSE$ \\
        %        \tab\tab nivelComercio($p$, borrar(dameUno(claves($cn$)), $cn$)) \\
        %    \tab $\LFI$ \\ 
        %$\LFI$
   ...
   %         $\emptyset$ 
   %     $\LELSE$ \\
   %         $\LIF$ distManhattan($p$, dameUno(claves($cs$))) $\leq$ 3 $\LTHEN$ \\
...
\tadAxioma{unirValido($a,\ b$)}{
    ($\forall$ $p$ : Pos)(def?($p$, construcciones($a$)) $\impluego$ $\\$
        ...
        \tab ($\nexists$\ $otra$ : Pos)(def?($otra$, construcciones($a$))\ $\yluego$ $\\$
    ...
    ($\forall$ $p$ : Pos)(def?($p$, construcciones($b$)) $\impluego$ $\\$
        ...
        \tab ($\nexists$\ $otra$ : Pos)(def?($otra$, construcciones($b$))\ $\yluego$ $\\$
