\begin{Interfaz}
    ...
    [$\bold{O}(casas^{2})$ $\supset$ $\Theta(\sum_{i = 0}^{N}{(hasta_i \times casas_i + camino_i)})$, 
        ...
        %\\ \underline{justificación abstracta:} Una implementación de SimCity debe poder, en el peor caso, retornar una copia de todas sus casas en tiempo lineal sobre el conjunto que las representa. Pero, dado que es esperable que un SimCity sea representado como un árbol, podemos suponer que para resolver colisiones del tipo 'se queda el primero', cada nueva casa a copiar debe primero evaluar si no fue ya copiada. Resultando en un peor caso O($casas^2$).
    ]
